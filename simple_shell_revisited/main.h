...
typedef struct Info_t
{
	...
	list_t *history;
	list_t *alias;
	list_t *env; // Info –env is a pointer variable holding the address of a self-refering struct with member (dir and num) and that points to another struct (which forms what we call a linked list).
} info_t;
...
#define HIST_FILE ".simple_shell_history" // the file where the history of command used in the shell by the user is stored
#define MAX_HISTORY 4096				  // the total number of command that can be stored in the history file
