...
{
	return (isatty(STDIN_FILENO) && information->readfd <= 2); // if stdin is true and the file descriptor is not above 2, then its true because if a file is opened then readfd will be above 2 and this means the user piped a file into our executable which is already opened
}
