...
{
	while (*needle) // while needle is not equal to the null-byte
	{
		if (*needle++ != *str++) // if the first values of both argument is not equal, it returns NULL, else it is incremented until needle becomes false
			return (NULL);
	}
	return ((char *)str); // returns the env variable
}
